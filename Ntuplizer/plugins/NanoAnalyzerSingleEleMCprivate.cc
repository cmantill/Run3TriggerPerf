...
#ifndef CMSSW12plus
// Run 1 and 2
class NanoAnalyzerSingleEleMCprivate : public edm::EDAnalyzer
#else
// Run 3
class NanoAnalyzerSingleEleMCprivate : public edm::one::EDAnalyzer<>
#endif
{
public:
  explicit NanoAnalyzerSingleEleMCprivate(const edm::ParameterSet&);
  ~NanoAnalyzerSingleEleMCprivate();
...
NanoAnalyzerSingleEleMCprivate::NanoAnalyzerSingleEleMCprivate(const edm::ParameterSet& iConfig)
{
...
NanoAnalyzerSingleEleMCprivate::~NanoAnalyzerSingleEleMCprivate() { }

///////////////////////////////////////////////////////////////////////////////
////////////// main analysis loop: method called for each event ///////////////
///////////////////////////////////////////////////////////////////////////////

void NanoAnalyzerSingleEleMCprivate::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  reset();
  ...
  for (unsigned int i = 0, n = HLTtriggers_->size(); i < n; ++i) {
    ...
    if(trigNames.triggerName(i).find("HLT_SingleEle8_SingleEGL1_v")!= std::string::npos){
      if(HLTtriggers_->accept(i)){
	      // isTriggered = true;
        std::cout << "SingleEle Fired" << std::endl;
  ...
  for (size_t iele =0; iele < electronpair.size(); iele++){
    ...
    eleTV3.SetPtEtaPhi( electronpair[iele].pt(), electronpair[iele].eta(), electronpair[iele].phi() );
  ...
  } //NanoAnalyzerSingleEleMCprivate::analyze ends
...
void NanoAnalyzerSingleEleMCprivate::beginJob(const edm::ParameterSet& iConfig)
{ }

// ------------ method called once each job just before starting run  ------------
void NanoAnalyzerSingleEleMCprivate::beginRun(const edm::Run &iRun, const edm::EventSetup &iStp)
{ }

void NanoAnalyzerSingleEleMCprivate::fillDescriptions(edm::ConfigurationDescriptions & descriptions) 
{ }

// ------------ method called when ending the processing of a run  ------------  Qun below
void NanoAnalyzerSingleEleMCprivate::endRun(edm::Run const&, edm::EventSetup const&)
{ }

void 
NanoAnalyzerSingleEleMCprivate::endJob()
{ }
...
void NanoAnalyzerSingleEleMCprivate::createBranch() { 
...
void NanoAnalyzerSingleEleMCprivate::reset(void){
...
DEFINE_FWK_MODULE(NanoAnalyzerSingleEleMCprivate);
